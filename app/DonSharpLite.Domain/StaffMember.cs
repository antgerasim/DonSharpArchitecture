...
{
    [HasUniqueDomainSignature(ErrorMessage = "A staff member already exists with the same employee number")]
    public class StaffMember : Entity
    {
        [DomainSignature]
        [Required(ErrorMessage = "Employee number must be provided")]
        [StringLength(6,MinimumLength = 6, ErrorMessage = "Employee number must be 6 characters long")]
